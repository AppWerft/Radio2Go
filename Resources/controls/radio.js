...
Radio.prototype.getHoerspieltipps = function(_weekoffset, _callback) {
	...
	moment.lang('de');
	var weeks = _weekoffset || 0;
	var kw = moment().add('weeks', weeks).format('wwYY');
	var yql = 'select * from html WHERE url="http://www.hoerspieltipps.net/Radio' + kw + '.html" and xpath="//tbody"';
	console.log(yql);
	Ti.Yahoo.yql(yql, function(_y) {
		if (_y.success) {
			var termine = [], row, rows = _y.data.tbody.tr;
			rows.shift();
			while ( row = rows.shift()) {
				if (!row.td[2] || !row.td[2].a || !row.td[2].a.img)
					continue;
				var moment = require('vendor/moment');
				var timestamp = moment(row.td[0].p + ' ' + row.td[1].p, "dddd, D. MMMM YYYY HH:mm", 'de');
				if (moment(timestamp).isAfter()) {
					termin = {
						datum : row.td[0].p,
						uhrzeit : row.td[1].p,
						sender : row.td[2].a && row.td[2].a.img.alt,
						logo : row.td[2].a && row.td[2].a.img.src,
						detaillink : row.td[2].a && row.td[3].a.href,
						titel : row.td[3].a && row.td[3].a.content.replace(/\n/g, ' ').replace(/\s+/g, ' '),
						autor : 'Autor: ' + row.td[4].p,
						produktion : 'Produktion: ' + row.td[5].p,
						dauer : 'Dauer: ' + row.td[6].p,
						bemerkung : row.td[7].p && row.td[7].p.content,
					};
					termine.push(termin);
				}
			}
...
Radio.prototype.getSendungen = function() {
	...
	res.close();
	var q = 'SELECT DISTINCT termine.*, sender.longname AS longname FROM termine,sender WHERE sender.id=termine.senderid AND wd=' + wd + ' ORDER BY start';
	var res = link.execute(q);
	while (res.isValidRow()) {
